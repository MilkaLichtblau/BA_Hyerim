...
def plot(dataName, data_set,attributeNamesAndCategories, attributeQuality):
...
#     pl.hist(round_2f,30, histtype='bar', label=label, color=colors[:len(categories)])    
    ...
    pl.legend(loc='center left', fontsize='x-large', bbox_to_anchor=(1, 0.5))
    pl.show()
    out_png = '../Plots/histo_plot_'+dataName+'.png'
    pl.savefig(out_png, dpi=100)    
    
def plot_scatter(dataName, data_set,attributeNamesAndCategories, attributeQuality):
#     label=['Germany','Turkey','Greece','Italy','Spain','France']
...
#     pl.ylim(0, 6.0)
    pl.show()              
    out_png = '../Plots/scatter_'+dataName+'.png'
    pl.savefig(out_png, dpi=100)   
...
def multinomFair(dataName, data_set, attributeNamesAndCategories, attributeQuality, k, p, alpha, color_blind):
    categories = determineGroups(attributeNamesAndCategories)
    num_categories = len(categories)
    ...
    target = ()
    attributeItems = attributeNamesAndCategories.items()
    fileName = dataName.replace(".json","_")+str(p)+'_'+str(k)
...
#             print item
    ...
    plot(fileName, ranking,attributeNamesAndCategories, attributeQuality)        
    plot_scatter(fileName, ranking,attributeNamesAndCategories, attributeQuality)
...
if __name__ == '__main__':
    ...
    multinomFair(dataName=args.data_set, data_set=data, attributeNamesAndCategories={args.attributeNames:args.categories},
